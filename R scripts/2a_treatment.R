...
load("C:/Users/16083/Documents/GitHub/PaperEduc/Data 2015/col_sample_data.RData")
...
##========================================================================##
col_teachers %>% select(CNTSCHID, CNTTCHID, TC001Q01NA) %>% group_by(CNTSCHID) %>% count(TC001Q01NA)
col_students %>% select(CNTSCHID, CNTSTUID, ST004D01T) %>% group_by(CNTSCHID) %>% count(ST004D01T)
...
prop_female_teach <- data.frame(100*prop.table(table(col_teachers$CNTSCHID, col_teachers$TC001Q01NA),1)) %>% spread(Var2, Freq )%>% mutate(Var1 = as.numeric(as.character(Var1)))
...
school_treated <- as.data.frame(100*prop.table(table(col_teachers$CNTSCHID, col_teachers$TC001Q01NA),1)) %>%
...
col_students <-left_join(col_students, school_treated, by="CNTSCHID") ## Merge The school Terated variables in Students Data.

# Generete the treatment for the female students.
col_students <- col_students %>% mutate(T_50 = ifelse((prop_fem_50==1 & ST004D01T==1),1,0),
...
col_students %>%   summarise(T_50 = sum(T_50),
                             T_75 = sum(T_75),
                             T_100 = sum(T_100))
...
a <- col_schools %>% group_by(CNT, CNTSCHID) %>% group_by(CNT) %>% count(CNT) 
#Number of Teachers per country.
b <- col_teachers %>% group_by(CNT, CNTSCHID, CNTTCHID) %>% group_by(CNT) %>% count(CNT)
#Number of Students per country. 
c <- col_students %>% group_by(CNT, CNTSCHID, CNTSTUID) %>% group_by(CNT) %>% count(CNT)
d <- left_join(a,b, by="CNT")
Totales_col <- left_join(d,c, by='CNT')
rm (a,b,c,d)
colnames(Totales_col) <- c("CNT", "Schools", "Teachers", "Students")

##Create a new variable Gender as a FACTOR and string, to easier graphs.
col_students <- col_students %>% mutate(Gender = ifelse(ST004D01T==1,"Female","Male"),
                                        Female = ifelse(ST004D01T==1,1,0))
schools = col_schools
students = col_students
teachers = col_teachers
save(schools, teachers, students, file = "col_treatment_data.RData")
rm (list = ls(all.names = TRUE))
load("C:/Users/16083/Documents/GitHub/PaperEduc/Data 2015/col_treatment_data.RData")
print("The Colombian Sample has been Imported")
